...
public:
    ...
    std::unique_ptr<FocalElement> intersect(const FocalElement& other) const{
        if(typeid(*this)!= typeid(other))throw IncompatibleTypeError("Intersection is not supported for focal elements of different type");
        return do_intersection(other);
    };
     std::unique_ptr<FocalElement> unite(const FocalElement& other) const{
         if(typeid(*this)!= typeid(other))throw IncompatibleTypeError("Union is not supported for focal elements of different type");
